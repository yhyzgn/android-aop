 ...
 * time   : 2019-04-05 18:03
 * version: 1.0.0
 * desc   : Aop工具类
 */
public class AopUtils {
    private static final String TAG = "AopUtils";
    ...
    public static <T> List<Class<? extends T>> getImplementations(Context context, String packageName, Class<T> clazz) {
        if (null != context && null != clazz) {
            List<Class<?>> allClass = getClassListInPackage(context, packageName);
            List<Class<? extends T>> result = new ArrayList<>();
            if (null != allClass && !allClass.isEmpty()) {
                for (Class<?> cz : allClass) {
                    if (clazz.isAssignableFrom(cz) && !cz.equals(clazz)) {
                        result.add((Class<? extends T>) cz);
                    }
                }
            }
            return result;
     ...
     * @throws InterruptedException                 线程中断异常
     */
    public static Set<String> getClassNameInPackage(Context context, String packageName, String classNamePrefix) throws PackageManager.NameNotFoundException, IOException, InterruptedException {
        if (null != context) {
